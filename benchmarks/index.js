...
var vtorrentd = require('../').services.Bitcoin({
  node: {
    datadir: process.env.HOME + '/.vtorrent',
    network: {
      name: 'testnet'
    }
  }
...
vtorrentd.on('ready', function() {
  ...
  async.series([
    function(next) {
      ...
      function vtorrentdGetBlockNative(deffered) {
        if (c >= hashesLength) {
          c = 0;
        }
      ...
      function vtorrentdGetBlockJsonRpc(deffered) {
        if (c >= hashesLength) {
          c = 0;
        }
        var hash = fixtureData.blockHashes[c];
      ...
      function bitcoinGetTransactionNative(deffered) {
        ...
        vtorrentd.getTransaction(hash, true, function(err, tx) {
          if (err) {
            throw err;
          }
          deffered.resolve();
      ...
      suite.add('vtorrentd getblock (native)', vtorrentdGetBlockNative, {
        defer: true,
        maxTime: maxTime
      });

      suite.add('vtorrentd getblock (json rpc)', vtorrentdGetBlockJsonRpc, {
        defer: true,
        maxTime: maxTime
      });

      suite.add('vtorrentd gettransaction (native)', bitcoinGetTransactionNative, {
        defer: true,
        maxTime: maxTime
      });

      suite.add('vtorrentd gettransaction (json rpc)', bitcoinGetTransactionJsonRpc, {
        defer: true,
        maxTime: maxTime
      });
