...
describe('#defaultBaseConfig', function() {
  it('will return expected configuration', function() {
    ...
    info.config.services.should.deep.equal(['vtorrentd', 'web']);
    var vtorrentd = info.config.servicesConfig.vtorrentd;
    vtorrentd.spawn.datadir.should.equal(home + '/.vtorrent');
    vtorrentd.spawn.exec.should.equal(path.resolve(__dirname, '../../bin/vtorrentd'));
  });
  ...
  it('be able to specify a datadir', function() {
    ...
    info.config.servicesConfig.vtorrentd.spawn.datadir.should.equal('./data2');
  });
});
